...
const VA_LEGAL_TERMS = [
  'service-connected', 'disability rating', 'C&P exam', 'compensation', 'pension',
  'VA medical center', 'veteran', 'combat veteran', 'Purple Heart', 'disability compensation',
  'individual unemployability', 'TDIU', 'PTSD', 'TBI', 'traumatic brain injury',
  'Gulf War syndrome', 'Agent Orange', 'burn pit', 'military sexual trauma', 'MST',
  'buddy statement', 'lay evidence', 'nexus letter', 'DBQ', 'disability benefits questionnaire',
  'effective date', 'clear and unmistakable error', 'CUE', 'secondary service connection',
  'aggravation', 'presumptive condition', 'combat stressor', 'chronicity', 'continuity',
  'in-service injury', 'service medical records', 'SMR', 'private medical evidence',
  'vocational rehabilitation', 'Chapter 31', 'Chapter 35', 'dependents education',
  'burial benefits', 'DIC', 'dependency and indemnity compensation', 'CHAMPVA',
  'fiduciary', 'representative payee', 'power of attorney', 'accredited representative'
]
...
export const useTranscription = (searchQuery = '', filterType = 'all'): UseTranscriptionReturn => {
  ...
  const processAudioFile = useCallback(async (file: File) => {
    try {
      ...
      if (result.success && result.transcript) {
        ...
        announceToScreenReader(`Transcription completed with ${Math.round(transcript.confidence * 100)}% confidence`)
  ...
  const exportTranscript = useCallback(async (transcript: Transcript, format: string) => {
    try {
      ...
      announceToScreenReader(`Export failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
      throw error
    }
  }, [])
  ...
  const searchTranscripts = useCallback((query: string): Transcript[] => {
    if (!query.trim()) return transcriptionState.transcripts

    const searchTerm = query.toLowerCase()
    return transcriptionState.transcripts.filter(t =>
  ...
  const extractKeyFindings = useCallback((transcript: string): string[] => {
    ...
    lines.forEach(line => {
      ...
      if (line.toLowerCase().includes('service-connected') ||
  ...
  const calculateConfidenceScore = useCallback((transcript: string): number => {
