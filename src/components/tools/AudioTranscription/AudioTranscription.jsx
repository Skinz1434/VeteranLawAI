  ...
  const mockTranscripts = [
    {
      ...
      language: 'en-US',
      summary:
        'Comprehensive PTSD evaluation discussing combat stressors, current symptoms, and functional impact. Patient reported nightmares, hypervigilance, and avoidance behaviors consistent with PTSD diagnosis.',
      keyFindings: [
...
[00:02:20] John Veteran: I've been seeing a therapist at the VA for two years now. The medication helps a little, but I still struggle daily. I had to quit my job last month because I couldn't function around all those people.`,
    ...
    },
    {
      ...
      language: 'en-US',
      summary:
        'Initial consultation regarding lumbar spine injury claim. Client seeking representation for service-connected back injury with secondary depression claim.',
      keyFindings: [
...
[00:01:40] Maria Rodriguez: They gave me 30% for my back. But I can't work a full day anymore. I've had three surgeries, and I'm on pain medication constantly. My doctor says I need another surgery, but the VA keeps denying it.`,
      ...
      mediaRecorderRef.current.onstop = () => {
        if (realTimeTranscript.trim()) {
          ...
          announceToScreenReader(
            `Recording completed. Transcript saved with ${transcript.keyFindings.length} key findings.`
          )
        }
      }
      ...
      try {
        ...
        if (result.success) {
          ...
          const transcript = {
            ...
            title:
              filename.replace(/\.[^/.]+$/, '') ||
              `Audio File - ${new Date().toLocaleDateString()}`,
            type: 'upload',
            date: timestamp,
            ...
            summary:
              result.summary ||
              'Audio transcription completed with VA legal terminology enhancement.',
          ...
          setTranscripts(prev => [transcript, ...prev])
          announceToScreenReader(
            `Transcription completed with ${Math.round(result.confidence * 100)}% confidence. ${transcript.keyFindings.length} key findings identified.`
          )
  ...
  const generateRecommendations = useCallback(transcript => {
    ...
    if (
      ...
      lowerTranscript.includes('employ')
    ) {
  ...
  return (
    ...
    >
                ...
                {serviceInfo && (
                  <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20 rounded-xl p-4 mb-4">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div
                        className={`flex items-center space-x-2 ${serviceInfo.capabilities.realTimeTranscription ? 'text-green-400' : 'text-red-400'}`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${serviceInfo.capabilities.realTimeTranscription ? 'bg-green-400' : 'bg-red-400'}`}
                        />
                        <span>Real-time Transcription</span>
                      </div>
                      <div
                        className={`flex items-center space-x-2 ${serviceInfo.capabilities.fileProcessing ? 'text-green-400' : 'text-red-400'}`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${serviceInfo.capabilities.fileProcessing ? 'bg-green-400' : 'bg-red-400'}`}
                        />
                        <span>File Processing</span>
                      </div>
                      <div
                        className={`flex items-center space-x-2 ${serviceInfo.capabilities.speakerDetection ? 'text-green-400' : 'text-red-400'}`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${serviceInfo.capabilities.speakerDetection ? 'bg-green-400' : 'bg-red-400'}`}
                        />
                        <span>Speaker Detection</span>
                      </div>
                      <div
                        className={`flex items-center space-x-2 ${serviceInfo.capabilities.vaTerminologyEnhancement ? 'text-green-400' : 'text-red-400'}`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full ${serviceInfo.capabilities.vaTerminologyEnhancement ? 'bg-green-400' : 'bg-red-400'}`}
                        />
              ...
              <div className="max-w-lg mx-auto">
                <div className="mb-8">
                  ...
                  <p className="text-slate-300 text-lg mb-8">
                    Upload your audio files for professional VA legal transcription with AI-powered
                    analysis
                  </p>
                </div>
                ...
                <div className="mt-8 grid grid-cols-2 gap-4 text-left">
                  {[
                    ...
                    {
                      icon: Users,
                      label: 'Multi-speaker detection',
                      desc: 'Automatic diarization',
                    },
          ...
          >
            {[
              ...
              return (
                <Card
                  key={index}
                  className="p-6 hover:border-emerald-500/30 transition-all duration-300"
                >
      ...
      <Modal isOpen={!!selectedTranscript} onClose={() => setSelectedTranscript(null)} size="lg">
        {selectedTranscript && (
          <div>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-white mb-2">{selectedTranscript.title}</h2>
