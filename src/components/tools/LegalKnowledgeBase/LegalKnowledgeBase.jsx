...
const LegalKnowledgeBase = () => {
  ...
  const performSearch = useCallback(
    async (query, filter = activeFilter) => {
      setIsSearching(true)
    ...
    >
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          ...
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${typeStyle.bg} ${typeStyle.color}`}
                          >
                        ...
                        <p className="text-slate-300 mb-4 leading-relaxed">{result.summary}</p>
      ...
      {!searchQuery && (
        ...
        >
          {[
            {
              title: 'Most Referenced Regulations',
              count: '2,341',
              icon: Scale,
              color: 'from-blue-500 to-blue-600',
            },
            {
              title: 'Recent Case Precedents',
              count: '156',
              icon: FileText,
              color: 'from-green-500 to-green-600',
            },
            {
              title: 'VA Manual Updates',
              count: '89',
              icon: BookOpen,
            ...
            return (
              <Card
                key={index}
                className="p-6 hover:border-amber-500/30 transition-all duration-300 cursor-pointer"
              >
                <div
                  className={`relative w-12 h-12 bg-gradient-to-br ${item.color} rounded-xl flex items-center justify-center mb-4 overflow-hidden`}
                >
      ...
      {/* Result Detail Modal */}
      <Modal isOpen={!!selectedResult} onClose={() => setSelectedResult(null)} size="lg">
        {selectedResult && (
          <div>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-white mb-2">{selectedResult.title}</h2>
