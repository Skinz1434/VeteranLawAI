...
const CameraOCR = () => {
  ...
  const [autoSaveToDrive, setAutoSaveToDrive] = useState(true)
  ...
  React.useEffect(() => {
    const initializeServices = async () => {
      try {
        ...
        const ocrResult = await initializeOCR()
        const ocrInfo = getOCRServiceInfo()
        setServiceInfo(ocrInfo)

        if (ocrResult.success) {
          announceToScreenReader(`OCR service initialized using ${ocrResult.engine}`)
        } else {
          console.error('OCR initialization failed:', ocrResult.error)
          announceToScreenReader('OCR service initialization failed')
        }

        // Initialize Google Drive service
        try {
          const driveResult = await initializeGoogleDrive()
          if (driveResult.success) {
            setGoogleDriveReady(true)
            announceToScreenReader('Google Drive integration ready')
          }
        } catch (driveError) {
          console.warn('Google Drive initialization failed:', driveError)
    ...
    async (input, filename = null) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (autoSaveToDrive && googleDriveReady) {
            try {
              const driveResult = await uploadToGoogleDrive(input, {
                folderName: 'OCR Results',
                description: `OCR processed: ${processedDoc.filename}`,
                tags: ['OCR', 'VA Form', processedDoc.type],
                metadata: {
                  confidence: processedDoc.confidence,
                  formType: processedDoc.metadata.formType,
                  vaTermsFound: processedDoc.metadata.vaTermsFound,
                },
              })

              if (driveResult.success) {
                processedDoc.googleDriveId = driveResult.fileId
                processedDoc.googleDriveLink = driveResult.webViewLink
                announceToScreenReader('Document automatically saved to Google Drive')
              }
            } catch (driveError) {
              console.warn('Failed to save to Google Drive:', driveError)
            }
          }
  ...
  const handleFileUpload = useCallback(
    async event => {
      ...
      try {
        // Validate file type
        const allowedTypes = serviceInfo?.supportedFormats || [
          'image/jpeg',
          'image/png',
          'application/pdf',
        ]
        ...
        if (file.size > maxSize) {
          alert(
            `File size must be less than ${maxSize / (1024 * 1024)}MB. Your file is ${Math.round((file.size / (1024 * 1024)) * 10) / 10}MB.`
          )
          return
        }
  ...
  return (
    <PageShell>
      <ToolWrapper
        ...
        description="Advanced VA Legal Document Processing & AI Recognition with 99.7% accuracy for VA forms and medical records"
        icon={Scan}
        ...
        features={[
          {
            ...
            description: 'Advanced machine learning for 99.7% accuracy',
          },
          {
            icon: Shield,
            title: 'VA Form Recognition',
            description: 'Automated detection of 21-526EZ, 21-4142, DBQ forms',
          },
          {
            icon: Cloud,
            title: 'Google Drive Integration',
            description: 'Automatic cloud backup and organization',
          },
          {
            icon: Zap,
            title: 'Real-time Processing',
            description: 'Instant results with live progress tracking',
          },
        ]}
        examples={[
          {
            title: 'VA Disability Claims',
            description: 'Process 21-526EZ forms with automatic field extraction',
            steps: [
              'Capture or upload the form',
              'AI detects form type automatically',
              'Extract personal and medical information',
              'Save to Google Drive with metadata',
            ],
          },
          {
            title: 'Medical Records',
            description: 'Digitize and organize medical documentation',
            steps: [
              'Upload medical records and DBQs',
              'OCR extracts relevant information',
              'Organize by case and date',
        ...
        onFullscreenToggle={fullscreen => setIsFullscreen(fullscreen)}
      >
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <span id="confidence-threshold-description" className="sr-only">
                        Set minimum confidence level for OCR text extraction. Higher values mean
                        more accuracy but may reject valid text.
                      </span>
                    </div>
                    ...
                    {serviceInfo && (
                      <div className="bg-gradient-to-r from-slate-700/30 to-slate-800/30 rounded-lg p-3">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div
                            className={`flex items-center space-x-2 ${serviceInfo.initialized ? 'text-green-400' : 'text-red-400'}`}
                          >
                            <div
                              className={`w-2 h-2 rounded-full ${serviceInfo.initialized ? 'bg-green-400' : 'bg-red-400'}`}
                            />
                            <span>OCR Engine</span>
                          </div>
                          <div
                            className={`flex items-center space-x-2 ${serviceInfo.capabilities.vaFormRecognition ? 'text-green-400' : 'text-red-400'}`}
                          >
                            <div
                              className={`w-2 h-2 rounded-full ${serviceInfo.capabilities.vaFormRecognition ? 'bg-green-400' : 'bg-red-400'}`}
                            />
                            <span>VA Form Detection</span>
                          </div>
                          <div
                            className={`flex items-center space-x-2 ${googleDriveReady ? 'text-green-400' : 'text-yellow-400'}`}
                          >
                            <div
                              className={`w-2 h-2 rounded-full ${googleDriveReady ? 'bg-green-400' : 'bg-yellow-400'}`}
                            />
                            <span>Google Drive</span>
                          </div>
                          <div
                            className={`flex items-center space-x-2 ${autoSaveToDrive ? 'text-green-400' : 'text-blue-400'}`}
                          >
                            <div
                              className={`w-2 h-2 rounded-full ${autoSaveToDrive ? 'bg-green-400' : 'bg-blue-400'}`}
                            />
                            <span>Auto-Save</span>
                          </div>
                        </div>
                        <p className="text-slate-400 text-xs mt-2">
                          {serviceInfo.vaFormsSupported} VA forms supported • Max:{' '}
                          {serviceInfo.maxFileSize}
                        </p>
                      </div>
                    )}
                      ...
                      <div className="space-y-3">
                        <label className="flex items-center space-x-2 cursor-pointer">
                          ...
                          <span className="text-slate-300 text-sm">
                            Automatically save processed documents to Google Drive
                          </span>
                        </label>
                        <div className="flex items-center space-x-2 text-xs">
                          <div
                            className={`w-2 h-2 rounded-full ${googleDriveReady ? 'bg-green-400' : 'bg-yellow-400'}`}
                          />
                          <span className={googleDriveReady ? 'text-green-400' : 'text-yellow-400'}>
                            {googleDriveReady ? 'Connected and ready' : 'Not connected'}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <Card className="p-6">
                  ...
                  <div className="space-y-4">
                    {[
                      {
                        icon: '📄',
                        title: 'Position Document',
                        desc: 'Place the entire document within the camera frame',
                      },
                      {
                        icon: '💡',
                        title: 'Good Lighting',
                        desc: 'Ensure adequate lighting without shadows or glare',
                      },
                      {
                        icon: '📐',
                        title: 'Keep Steady',
                        desc: 'Hold the camera steady for clear, sharp images',
                      },
                      {
                        icon: '🎯',
                        title: 'Focus',
                        desc: 'Wait for the camera to focus before capturing',
                      },
                    ].map((tip, index) => (
                  ...
                  <Card key={doc.id} className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div className="flex space-x-2">
                        ...
                        <Button size="sm" variant="outline" onClick={() => exportDocument(doc)}>
                        ...
                        {doc.googleDriveLink && (
                          <Tooltip content="View in Google Drive" side="top">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => window.open(doc.googleDriveLink, '_blank')}
            ...
            >
              {[
                ...
                return (
                  <Card
                    key={index}
                    className="p-6 hover:border-cyan-500/30 transition-all duration-300"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div
                        className={`relative w-12 h-12 bg-gradient-to-br ${metric.color} rounded-xl flex items-center justify-center overflow-hidden`}
                      >
        ...
        <Modal isOpen={!!selectedDocument} onClose={() => setSelectedDocument(null)} size="lg">
          {selectedDocument && (
            <div>
              <h2 className="text-2xl font-bold text-white mb-4">{selectedDocument.filename}</h2>
