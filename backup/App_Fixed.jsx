...
import { 
  ...
  CheckCircle, Star, ArrowRight, Zap, Database, Brain,
  Award, TrendingUp, DollarSign, Target
} from 'lucide-react'
...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900">
      <Routes>
        <Route path="/" element={<HomePage scrollToSection={scrollToSection} showMobileMenu={showMobileMenu} setShowMobileMenu={setShowMobileMenu} showUserMenu={showUserMenu} setShowUserMenu={setShowUserMenu} user={user} signOut={signOut} />} />
        <Route path="/camera-ocr" element={<CameraOCR />} />
        <Route path="/legal-knowledge" element={<LegalKnowledgeBase />} />
        <Route path="/claim-guidance" element={<ClaimGuidance />} />
        <Route path="/audio-transcription" element={<AudioTranscription />} />
        <Route path="/case-research" element={<CaseResearch />} />
        <Route path="/analytics" element={<ClaimAnalytics />} />
      </Routes>
    </div>
  )
}

function HomePage({ scrollToSection, showMobileMenu, setShowMobileMenu, showUserMenu, setShowUserMenu, user, signOut }) {
  return (
    <>
      {/* Header */}
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="group relative flex items-center space-x-4">
              <div className="relative">
                ...
                />
                <div className="hidden h-12 w-12 bg-gradient-to-br from-cyan-400 via-blue-500 to-indigo-600 rounded-xl items-center justify-center shadow-lg">
            ...
            <nav className="hidden lg:flex items-center space-x-2">
              ...
              >
                <span className="relative z-10 font-medium">Experts</span>
              ...
              <button 
                ...
                className="group relative px-6 py-3 text-gray-300 hover:text-white transition-all duration-300 rounded-xl hover:bg-gradient-to-r hover:from-pink-500/10 hover:to-cyan-500/10 border border-transparent hover:border-pink-500/20"
              >
                <span className="relative z-10 font-medium">Pricing</span>
                <div className="absolute inset-0 bg-gradient-to-r from-pink-500/5 to-cyan-500/5 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
              </button>
            </nav>
